...
public sealed class LaunchMonitorDevice:BaseDevice {
 ...
 public override void HandleProtobufRequest(IMessage request){if(request is not WrapperProto w||w.Event?.Notification==null)return;var n=w.Event.Notification;var d=n.AlertNotification;if(d?.State!=null){CurrentState=d.State.State_;if(CurrentState==StateType.Standby&&AutoWake)WakeDevice();}if(d?.Error!=null&&d.Error.HasCode)Error?.Invoke($"{d.Error.Code}: {d.Error.Severity}");if(d?.Metrics!=null&&!processed.Contains(d.Metrics.ShotId)){processed.Add(d.Metrics.ShotId);ShotReceived?.Invoke(d.Metrics);}if(d?.TiltCalibration!=null)DeviceTilt=GetDeviceTilt();}
